...
class FSDocMetaPaths:
    ...
    def __init__(self, latest_versions_path: str, original_versions_path: str) -> None:
        ...
        if not self.latest_versions_path or not self.latest_versions_path.exists():
            raise ValueError(
                "Path to latest .abs versions " f"{latest_versions_path} does not exist"
            )
        if not self.latest_versions_path.is_dir():
            raise ValueError(
                "Path to latest .abs versions"
    ...
    def _get_parent_path(
        ...
        ver_path = (
            self.latest_versions_path if not version else self.original_versions_path
        )

        return ver_path.joinpath(
            (
                "arxiv"
                if not identifier.is_old_id or identifier.archive is None
