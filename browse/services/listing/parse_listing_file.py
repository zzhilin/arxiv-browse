...
REPLACED_FORM = re.compile(
    r"^(replaced with revised version)|^((figures|part) added)", re.IGNORECASE
)
...
def get_updates_from_list_file(
    year: int,
    month: int,
    listingFilePath: FileObj,
    ...
    while line:
        ...
        if article:
            ...
            if not listingFilter or (
                re.match(f"^{listingFilter}", primary) and neworcross == "new"
            ):
                # push update
                item = ListingItem(
                    id=article.arxiv_id,
                    listingType=neworcross,
                    primary=primary,
                    article=article,
                )
                if neworcross == "new":
    ...
    pub_dates_with_count: List[Tuple[date_class, int]] = []
    index = 0
    ...
    if parsingMode == "monthly_counts":
        ...
        return MonthCount(
            ...
            expires=gen_expires(),
            listings=new_items + cross_items + rep_items,
        )
    else:
        ...
        return Listing(
            listings=new_items + cross_items,
...
def _parse_item(item_lines: List[str]) -> Tuple[DocMetadata, str]:
    ...
    if fieldms:
        fields = {
            fieldm.group("field_name"): fieldm.group("value").replace("\n  ", " ")
