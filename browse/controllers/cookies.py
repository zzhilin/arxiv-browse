...
cookies_config = [
    ...
    mirror_config,
    {
        ...
        "options": [["enabled", "enabled", 1], ["disabled", "disabled", 0]],
    },
]
...
def get_cookies_page(is_debug: bool) -> Any:
    ...
    response_headers = {"Expires": "0", "Pragma": "no-cache"}
    return response_data, 200, response_headers


def selected_options_from_request(
    ...
    for cc in configs:
        ...
        matching_opt = next(
            (opt for opt in cc["options"] if opt[0] == request_value), None
        )
