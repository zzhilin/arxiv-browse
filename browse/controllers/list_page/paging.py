...
def paging(
    count: int, skipn: int, shown: int, context: str, subcontext: str
    ...
    prebumper = [
        n
        for n in page_starts
        if n >= (skipn - shown * bumper_pages) and n < skipn and n > 0
    ]

    if prebumper:
        if prebumper[0] <= shown * bumper_pages:
            # Case of no dots between first and prebumper
            page_links = page_links + [page_dict(n) for n in prebumper]
        else:
    ...
    postbumper = [
        n for n in page_starts if n > skipn and n <= (skipn + shown * bumper_pages)
    ]
    if postbumper:
        page_links = page_links + [page_dict(n) for n in postbumper]
