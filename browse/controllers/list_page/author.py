...
def get_json(id: str) -> Optional[Dict]:
    ...
    return {
        ...
        "modified": str(
            datetime.combine(datetime.today(), time.min, timezone.utc).isoformat()
        ),
        "orcid": orcid,
    ...
    def author_query(article: DocMetadata, query: str) -> str:
        try:
            if article.primary_archive:
                ...
                archive = Category[article.primary_category.id]["in_archive"]
            return str(
                url_for(
                    "search_archive", searchtype="author", archive=archive, query=query
                )
            )
...
def _guess_pub_date(metadata: DocMetadata) -> Optional[str]:
    ...
    if (
        metadata.arxiv_identifier.year is None
        or metadata.arxiv_identifier.month is None
    ):
        return None
...
def _make_json_entry(metadata: DocMetadata) -> Dict[str, str]:
    ...
    # 'authors' field
    entry["authors"] = ", ".join(
        map(_author_name, parse_author_affil(metadata.authors.raw))
    )
    ...
    if metadata.primary_category:
        ...
        if metadata.secondary_categories:
            all_categories.extend(
                map(lambda cat: cat.display, metadata.secondary_categories)
            )
    ...
    entry["formats"] = {
        ...
        "pdf": url_for(
            "dissemination.pdf", arxiv_id=metadata.arxiv_id_v, _external=True
        ),
    }
    ...
    entry["published"] = (
        pubdate.isoformat() if pubdate is not None else _guess_pub_date(metadata)
    )
...
def _add_atom_feed_entry(
    metadata: DocMetadata, feed: Element, atom2: bool = False
) -> None:
        ...
        for author_line in parse_author_affil(metadata.authors.raw):
            ...
            if affils:
                for affil in affils:
                    SubElement(
                        author,
                        QName(ARXIV_SCHEMA_URI, "affiliation"),
                        nsmap={"arxiv": ARXIV_SCHEMA_URI},
                    ).text = affil
    if metadata.doi:
        SubElement(
            entry, QName(ARXIV_SCHEMA_URI, "doi"), nsmap={"arxiv": ARXIV_SCHEMA_URI}
        ).text = metadata.doi
    if metadata.comments:
        SubElement(
            entry, QName(ARXIV_SCHEMA_URI, "comment"), nsmap={"arxiv": ARXIV_SCHEMA_URI}
        ).text = metadata.comments
    if metadata.journal_ref:
        SubElement(
            entry,
            QName(ARXIV_SCHEMA_URI, "journal_ref"),
            nsmap={"arxiv": ARXIV_SCHEMA_URI},
    ...
    SubElement(
        entry,
        QName(ARXIV_SCHEMA_URI, "primary_category"),
        attrib={
...
def _get_atom_feed(id: str, atom2: bool = False) -> str:
    ...
    SubElement(
        feed, "title"
    ).text = f"{get_user_display_name(user_id)}'s articles on arXiv"
    SubElement(
        feed,
        "link",
        attrib={
            "rel": "describes",
            "href": (
                f"{ORCID_URI_PREFIX}/{unquote(id)}"
                if is_orcid
                else _get_orcid_uri(user_id)
            ),
        },
    )
    # TODO: May need to add timezone info
    SubElement(feed, "updated").text = str(
        datetime.combine(datetime.today(), time.min, timezone.utc).isoformat()
    )
    ...
    SubElement(
        feed, "link", attrib={"rel": "describes", "href": f"{request.url_root}{id}"}
    )
