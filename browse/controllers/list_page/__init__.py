...
def get_listing(
    subject_or_category: str, time_period: str, skip: str = "", show: str = ""
) -> Response:
    """
    ...
    if not subject_or_category or not (
        time_period
        and (
            time_period.isdigit()
    ...
    if time_period == "new":
        list_type = "new"
        new_resp: Union[
            ListingNew, NotModifiedResponse
        ] = listing_service.list_new_articles(
        ...
        if resp.pubdates and resp.pubdates[0]:
            ...
            response_data[
                "pubmonth"
            ] = datetime.now()  # just to make the template happy
    ...
    response_data.update(
        {
            ...
            "paging": paging(count, skipn, shown, subject_or_category, time_period),
...
def dl_for_articles(items: List[Any]) -> Dict[str, Any]:
    ...
    return {
        item.article.arxiv_id_v: formats_from_source_type(
            _src_code(item.article), dl_pref
        )
        for item in items
    }
...
def latexml_links_for_articles(listings: List[Any]) -> Dict[str, Any]:
    ...
    return {
        item.article.arxiv_id_v: get_latexml_url(item.article, True)
        for item in listings
    }
...
def author_links(abs_meta: DocMetadata) -> Tuple[AuthorList, AuthorList, int]:
    ...
    return split_long_author_list(queries_for_authors(raw), truncate_author_list_size)


def index_for_types(
    resp: ListingNew, context: str, subcontext: str, skipn: int, shown: int
) -> Dict[str, Any]:
...
def sub_sections_for_types(resp: ListingNew, skipn: int, shown: int) -> Dict[str, Any]:
    ...
    for sec in secs:
        ...
        tot = sec["total"]
        sec["heading"] = (
            f"{typ} submissions for {date} " f"({showing}{n} of {tot} entries )"
        )
...
def _expires_headers(
    listing_resp: Union[Listing, ListingNew, NotModifiedResponse]
) -> Dict[str, str]:
