...
def get_latexml_url(article: DocMetadata, most_recent: bool = False) -> Optional[str]:
    ...
    status = (
        get_latexml_status_for_document(article.arxiv_id, article.highest_version())
        if most_recent
        else get_latexml_status_for_document(article.arxiv_id, article.version)
    )
    logging.debug(
        f"{article.arxiv_id_v} version: {article.version}, highest_version: {article.highest_version()}"
    )
